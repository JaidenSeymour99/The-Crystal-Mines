...
public class Player1 : Character
{
    ...
    {
        base.Update();
        if(state.isDashing || state.isWallJumping) return;
    ...
    public override void FixedUpdate()
    {
        ...
        state.rb.velocity = new Vector2(direction * speed, state.rb.velocity.y);
    ...
    {
        state.myAnimator.SetBool("die", true);
        base.DisableOnDeath();
        yield return null;
    ...
    {
        state.myAnimator.SetFloat("speed", Mathf.Abs(direction));
    ...
    private void IsAttacking()
    {
        if(state.attacking)
        {
            state.myAnimator.SetTrigger("attack1");
        }
    }

    private void IsWalling()
    {
        if (state.isWallSliding && state.isJumping)
        {
            state.myAnimator.SetTrigger("jump");
            state.myAnimator.SetBool("walled", false);
        }
        else if (state.isWallSliding)
        {
            state.myAnimator.SetBool("walled", true);
    ...
    {
        if (state.isJumping || state.isWallJumping && !state.attacking)
        {
            state.myAnimator.SetTrigger("jump");
            state.myAnimator.SetBool("walled", false);
        }
        else if (state.isJumping && !state.attacking)
        {
            state.myAnimator.SetTrigger("jump");
        ...
        else 
        {
            ...
            // speed = maxSpeed;
            state.myAnimator.ResetTrigger("jump");
        }
    }
    private void IsFalling()
    {
        if (state.isFalling && state.isWallSliding && !state.attacking)
        {
            state.myAnimator.SetBool("walled", true);
            state.myAnimator.SetBool("falling", true);
        }
        else if (state.isFalling && !state.attacking)
        {
            state.myAnimator.SetBool("falling", true);
        }
        else
        {
            state.myAnimator.SetBool("falling", false);
        }
    }
